...
class HttpReportClient(object):
    ...
    def __add_cup_info(self):
        ...
        if lower_env_label.find('linux') != -1:
            cpu_info = os.popen('cat /proc/cpuinfo | grep "models name" | uniq').read().split(':')[1].rstrip(
                '\n').strip()
        ...
        with self.__lock:
            err_msg = {
                ...
                'attributes': {
                    ...
                    'host': self.host_info,
                    'level': level,
                    'service': self.parent.token['service'] if 'service' in self.parent.token else 'service',
                    'sdk_ver': self.parent.sdk_version,
                    'env': self.env_label,
                    'ts': round(time.time() * 1000),
                    'code': code,
